...
class GoogleClient extends BaseClient {
  ...
  createLLM(clientOptions) {
    ...
    if (this.project_id != null) {
      logger.debug(`[GoogleClient Debug] Creating VertexAI client for model: ${model}`);
      ...
      return client;
    // } else if (!EXCLUDED_GENAI_MODELS.test(model)) { // Commented out exclusion logic
    } else { // Default to new @google/genai client if project_id is not set
      logger.debug(`[GoogleClient Debug] Creating GenAI client for model: ${model}`);
      const genAI = new GenAI(this.apiKey, requestOptions);
      return genAI.getGenerativeModel({ model });
    }

    /* // Commented out fallback to older Langchain client
    logger.debug(`[GoogleClient Debug] Creating ChatGoogleGenerativeAI (Langchain) client for model: ${model}`);
  ...
  async getCompletion(_payload, options = {}) {
    ...
    try {
      if (!EXCLUDED_GENAI_MODELS.test(modelName) && !this.project_id) {
        logger.debug(`[GoogleClient Debug] Using GenAI client for completion (model: ${modelName})`);
        /** @type {import('@google/genai').GenerativeModel} */
        const client = this.client;
        /** @type {import('@google/genai').GenerateContentRequest} */
        ...
        /** @type {import('@google/genai').UsageMetadata} */
        let usageMetadata;
        ...
        for await (const chunk of result.stream) {
          logger.debug('[GoogleClient Debug] Initial stream chunk:', chunk); // Log initial chunk
          ...
          if (chunk.functionCalls && chunk.functionCalls.length > 0) {
            logger.debug('[GoogleClient Debug] Function calls in initial chunk:', chunk.functionCalls); // Log function calls in initial chunk
            // Store all function calls from this chunk
            for (const call of chunk.functionCalls) {
              logger.debug('[GoogleClient] Function call received in chunk:', call);
          ...
          if (chunkText) {
            ...
            reply += chunkText;
            // await sleep(streamRate); // Removed artificial delay
          }
        }
  
        // After the stream finishes, process the final usage metadata and potential function call
        if (usageMetadata) {
          this.usage = {
            input_tokens: usageMetadata.promptTokenCount,
            output_tokens: usageMetadata.candidatesTokenCount,
          };
        // If function calls were received during the stream, process them sequentially
        if (functionCalls.length > 0) {
          logger.debug('[GoogleClient] Stream finished with function calls:', functionCalls);
          
          // Keep track of all messages for the conversation
          let updatedContents = [..._payload];
          let fullReply = reply;
          
          // Initialize followupRequestOptions before the loop
          const followupRequestOptions = {
            ...requestOptions,
            contents: updatedContents // Initial contents before any tool calls
          };
          ...
          for (const functionCall of functionCalls) {
            ...
            try {
              ...
              toolOutput = await processGeminiFunctionCall({ functionCall, client: this });
              logger.debug('[GoogleClient] Tool output received:', toolOutput);
            } catch (toolError) {
              logger.error('[GoogleClient] Error processing function call:', toolError);
              toolOutput = {
                name: functionCall.name,
                response: {
                  content: `Error executing tool "${functionCall.name}": ${toolError.message}`
                }
              };
              toolErrorOccurred = true;
            }

            // Format the tool output (or error) for Gemini API as a user message
            ...
            // Add the tool response (or error) to the conversation
            updatedContents.push(toolCallMessage);

            // If a tool error occurred, we might not want to continue with a follow-up API call for this tool.
            // However, to maintain the interleaved flow and allow the model to see the error,
            // we will still include it in the updatedContents and proceed with the follow-up call.
            // The model should ideally handle the error response.

            // Send the tool output (or error) back to Gemini in a subsequent API call
            logger.debug('[GoogleClient] Sending tool output (or error) back to Gemini:', toolCallMessage);

            // Update followupRequestOptions with the latest contents
            followupRequestOptions.contents = updatedContents;
            ...
            try {
              ...
              for await (const chunk of followupResult.stream) {
                logger.debug('[GoogleClient Debug] Follow-up stream chunk:', chunk); // Log follow-up chunk
                // Accumulate usage metadata from chunks
                usageMetadata = !usageMetadata
                  ? chunk?.usageMetadata
                  : Object.assign(usageMetadata, chunk?.usageMetadata);
                ...
                if (chunk.functionCalls && chunk.functionCalls.length > 0) {
                  logger.debug('[GoogleClient Debug] Function calls in follow-up chunk:', chunk.functionCalls); // Log function calls in follow-up chunk
                  for (const call of chunk.functionCalls) {
              ...
              logger.error('[GoogleClient] Error processing follow-up stream:', followupError);
              // Decide how to handle follow-up stream errors:
              // - Attempt to continue with the next function call?
              // - Terminate the current turn gracefully?
              // For now, we'll log and break the loop.
              break; // Exit the loop on follow-up stream error
            }
          }
      ...
      logger.debug(`[GoogleClient Debug] Using ChatVertexAI/ChatGoogleGenerativeAI client for completion (model: ${modelName})`);
